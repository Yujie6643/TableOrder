...
MODEL_PATH = f"/data/tyj/2D-TPE-main/output/{DATASET_NAME}_2d"
INPUT_PATH = f"/data/tyj/2D-TPE-main/eval_data/{DATASET_NAME}_test.json"
OUTPUT_PATH = f"/data/tyj/2D-TPE-main/res/{DATASET_NAME}_res.json"
sample_size = int(os.environ.get("TEST_SAMPLE_SIZE", 2000))
...
def read_data(input_file, to_tokenize_queue, sample_size=sample_size, seed=42):
    with open(input_file, "r") as f:
        ds = json.load(f)

    print(f"original size: {len(ds)}")

    # 随机采样 20000 条；如果不足 20000 条，就全量使用
    if len(ds) > sample_size:

        random.seed(seed)
        ds = random.sample(ds, sample_size)

    print(f"sampled size: {len(ds)}")
