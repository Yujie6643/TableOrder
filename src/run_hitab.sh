...
# ===== 路径设置 =====
# 最终模型目录；load_best_model_at_end=True 时根目录保存验证集最优模型。
OUTPUT_DIR=${OUTPUT_DIR:-/data/tyj/2D-TPE-main/output/hitab_${TABLE_READ_MODE}}
# 可选的表格块导出文件，用于检查生成的表格块。
TABLE_BLOCKS_DUMP_PATH=${TABLE_BLOCKS_DUMP_PATH:-/data/tyj/2D-TPE-main/output/hitab_table_blocks_6_999_${TABLE_READ_MODE}.json}
...
TRAIN_ARGS=(
        ...
        --data_path /data/tyj/2D-TPE-main/data/hitab_train_7417.json
